...
- Each record: **browser** requested **what URL** at **what time**
...
\begin{tikzpicture}[
  node distance=1.8cm,
  box/.style={font=\bfseries\large, align=center},
  arr/.style={-{Stealth[length=3mm]}, thick}
]
  % Top row
  \node[box] (traces) {Traces};
  \node[box, right=of traces] (visits) {Visits};
  \node[box, right=of visits] (sequences) {Sequences};
  \node[box, right=of sequences] (sessions) {Sessions};

  \draw[arr] (traces) -- (visits);
  \draw[arr] (visits) -- (sequences);
  \draw[arr] (sequences) -- (sessions);

  % Bottom branch from Visits
  \node[box, below=1.2cm of visits, draw, dashed, rounded corners, inner sep=4pt] (content) {Content};
  \node[box, right=of content] (category) {Classification};

  \draw[arr] (visits) -- (content);
  \draw[arr] (content) -- (category);
\end{tikzpicture}
\end{center}
...
- `adaR` -- robust URL parsing

\vspace{0.5em}

- `tidyverse` -- general data manipulation and visualization
- `here` -- Ensure file paths work across different environments
...
https://is.gd/wt_tutorial


\begin{center}
\includegraphics[width=0.3\textwidth]{../assets/qrcode_material.png}
\end{center}
