...
```{r visualize-data-visits-by-weekday, fig.width=8, fig.height=5}
# Visits by day of week
visits_by_weekday <- wt_data %>%
  mutate(
  ...
  count(weekday, name = "visits")

ggplot(visits_by_weekday, aes(x = weekday, y = visits)) +
  geom_col(fill = "steelblue") +
  scale_y_continuous(labels = function(x) format(x, big.mark = ",", scientific = FALSE)) +
  labs(
...
Raw web tracking data contains artifacts from browser behavior and the tracking system itself. Consecutive duplicate visits (e.g., from page refreshes or redirects), extremely short visits, and other noise need to be cleaned before analysis. Let's first look at what these artifacts look like in our data.
...
Notice how some URLs appear multiple times in a row. These are typically browser refreshes, redirects, or tracking artifacts rather than genuine separate visits. Before we can identify and remove these duplicates, we need to know how much time elapsed between consecutive visits.
...
cat("Records before cleaning:", format(nrow(wtr_data), big.mark = ","), "\n\n")
...
We can see a large spike at very short durations. Many of these are consecutive visits to the same URL -- browser artifacts that we want to remove. The `deduplicate()` function from webtrackR identifies consecutive visits to the same URL within a time window and aggregates them into a single visit:

```{r deduplicate}
# Deduplicate consecutive visits to the same URL within 1 second
wtr_data <- deduplicate(wtr_data, method = "aggregate", within = 1, keep_nvisits = TRUE)

cat("Records after deduplication:", format(nrow(wtr_data), big.mark = ","), "\n")
```
...
Let's see how much data we removed and how the duration distribution changed after cleaning:
...
**Try it yourself:** What happens if you change the deduplication window from 1 second to 5 seconds? How does it affect the number of records?
...
Note that `classify_visits()` with `match_by = "domain"` performs exact domain matching. Visits to domains not in our list get `NA`. This is more precise than regex but requires listing each domain explicitly. For a comprehensive classification, you would build a larger domain list -- or combine both approaches.

```{r classify-check}
# Verify classification by showing sample URLs from each category
cat("=== Sample URLs by Category ===\n")
wtr_data %>%
  mutate(type = replace_na(type, "Other/Unclassified")) %>%
  group_by(type) %>%
  slice_sample(n = 3) %>%
  select(type, domain, url) %>%
  ungroup() %>%
  arrange(type) %>%
  print(n = 20)
```

Let's visualize the distribution of visits across categories:

```{r classify-viz, fig.width=8, fig.height=5}
wtr_data %>%
  mutate(type = replace_na(type, "Other/Unclassified")) %>%
  count(type, sort = TRUE) %>%
  ...
  scale_y_continuous(labels = function(x) format(x, big.mark = ",", scientific = FALSE)) +
  labs(
    title = "Web Tracking Visits by Category",
    x = NULL,
    y = "Number of Visits"
  ) +
  theme_minimal()
