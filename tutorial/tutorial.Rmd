...
These are typically browser refreshes, redirects, or tracking artifacts rather than genuine separate visits. Before we can identify and remove these duplicates, we need to know how much time elapsed between consecutive visits.
...
We can see a large spike at very short durations. Many of these are consecutive visits to the same URL -- browser artifacts that we want to remove. The `deduplicate()` function from webtrackR identifies consecutive visits to the same URL within a time window and drops them:
...
**Try it yourself:** What happens if you change the deduplication window from 2 second to 5 seconds? How does it affect the number of records?
...
survey_data <- read.csv2("data/ZA5670_survey_data.csv")
