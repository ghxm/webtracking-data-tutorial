...
We can also clean up URLs by removing query parameters and fragments.
This is useful for grouping visits to the same page regardless of
tracking parameters (e.g., `?utm_source=google`):
...
**Try it yourself:** Can you subdivide news visits into subcategories
(politics, sports, culture) using the `path` column? Hint: use
`str_detect(path, "politik|political")` patterns on the path component.
