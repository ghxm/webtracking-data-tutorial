...
We can also use `adaR` to check (for components) and modify (components of) URLs. For more information, see the [adaR documentation](https://gesistsa.github.io/adaR/).
...
Note: In the above calculation we don't consider days where participants did no browsing at all.


We can also take a look at the duration of sessions:

```{r session-distribution, fig.width=8, fig.height=5}

sessions_duration <- wtr_data %>%
  group_by(panelist_id, session) %>%
  summarise(session_duration_minutes = sum(duration, na.rm = TRUE)/60, .groups = "drop")

ggplot(sessions_duration, aes(x = session_duration_minutes)) +
  geom_histogram(bins = 50, fill = "steelblue", color = "white") +
  labs(
    title = "Distribution of Session duration (30-min cutoff)",
